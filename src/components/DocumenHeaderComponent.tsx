...
const DocumentHeaderComponent: React.FC<DocumentHeaderComponentProps> = (props: DocumentHeaderComponentProps) => {
    ...
    const handlerGoToEdit = () => {
        navigate(location.pathname + "/edit", { state: props.data });
    }

    const handlerGoToCreate = () => {
        navigate(location.pathname.replace(id ?? '', 'create'));
    }
    ...
    return <div className='flex justify-between items-center bg-white p-2 rounded-lg px-6 shadow-sm'>
        <div className='flex gap-2 items-center'>
            <BackButton />
            <h1 className='font-bold capitalize'>{location.pathname.split('/')[2].replace('-', ' ')}</h1>
        ...
        <div className='text-[12px] flex gap-3'>
            <div role="button" onClick={handlerGoToEdit} className=" hover:bg-gray-200 hover:shadow-sm rounded-lg p-2 px-3 border hover:text-blue-500 text-[12px]">Edit</div>
            <div role="button" onClick={handlerCopyTo} className=" hover:bg-gray-200 hover:shadow-sm rounded-lg p-2 px-3 border hover:text-blue-500 text-[12px]">Copy To</div>
            <div role="button" onClick={handlerGoToCreate} className=" hover:bg-gray-200 hover:shadow-sm rounded-lg p-2 px-3 text-base border hover:text-blue-500"><HiOutlineDocumentAdd className="" /></div>
