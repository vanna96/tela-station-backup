...
export default function RouteMasterList() {
  const route = useNavigate();



  const { data, isLoading }: any = useQuery({
    queryKey: ["route-master"],
    queryFn: () => new RouteMasterRepository().get(),
  });
...

      {
        ...
        Cell: (cell: any) => (
          <div className="flex gap-4">
            <button onClick={() => {
              route('/logistic/route-master/' + cell.row.original.id, { state: cell.row.original })
            }}>
      ...
      <div className="w-full h-full p-4 2xl:py-6 flex flex-col gap-3 relative bg-gray-100">
        <div className="flex px-8 shadow-sm rounded-lg justify-between items-center sticky z-10 top-0 w-full bg-white py-3">
          ...
          <Button
            ...
            disableElevation
            size="small"
            onClick={() => route("/logistic/route-master/create")}
          >
