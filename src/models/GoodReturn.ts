...
export interface GoodReturnProps {
  id: any;
  docNum: any;
  cardCode?: string;
  cardName?: string;
  ...
  documentLine: GoodReturnDocumentLineProps[];
  requiredDate: string;
...
export interface GoodReturnDocumentLineProps {
...
export default class GoodReturn extends Model implements MasterDocument {
  ...
  constructor(json: any) {
    ...
    this.items = json["DocumentLines"]?.map(
      (e: any) => new GoodReturnDocumentLine(e)
    );
    ...
    this.accountNameD = new GLAccountRepository().find(
      json["AccountCode"]
    )?.Name;
  ...
  public static toCreate(json: any) {
    ...
    return {
      ...
      DocumentLines: json["items"].map((e: any) =>
        GoodReturnDocumentLine.toCreate(e, json["docType"])
      ),
    };
  }
